"""
Secure Surf Zone — Telegram Shop Bot (Admin Panel Included)
===========================================================

How to ACTIVATE (Render):
1) Upload this index.py + requirements.txt to GitHub
...
6) Telegram -> open bot -> /start
------------------------------------
Keep 24/7 (Free method):
Use UptimeRobot to ping your Render URL every 5 minutes:
https://your-app-name.onrender.com/
(Free tier may still sleep sometimes, but this helps.)
"""
...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        seed = [
            ("Adobe Explore (1 Month) — Available", 250, 300, 30, "Adobe Explore premium access. Delivery after verification."),
...
async def on_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin:"):
        ...
        # Send template commands (easy for phone)
        if action == "addp":
            await q.edit_message_text(
                "➕ Add Product Template:\n\n"
                "/addp Name | price | stock | duration_days | description\n\n"
                "Example:\n"
                "/addp Netflix (1 Month) | 350 | 50 | 30 | Netflix premium delivery after verification"
            )
            return

        if action == "price":
            await q.edit_message_text("💰 Update Price Template:\n\n/price <product_id> <new_price>\nExample:\n/price 1 550")
            return

        if action == "stock":
            await q.edit_message_text("📦 Update Stock Template:\n\n/stock <product_id> <new_stock>\nExample:\n/stock 1 999")
            return

        if action == "duration":
            await q.edit_message_text("⏳ Update Duration Template:\n\n/duration <product_id> <days>\nExample:\n/duration 1 60")
            return

        if action == "delp":
            await q.edit_message_text("🗑 Delete Product Template:\n\n/delp <product_id>\nExample:\n/delp 7")
            return
...
async def checkout_trx(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Status: Pending (verification চলবে)",
        reply_markup=kb_main(),
    )
...
async def cmd_addp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pid = cur.lastrowid
    con.close()
    await update.message.reply_text(f"✅ Added product #{pid}")
...
async def cmd_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    con.commit()
    con.close()
    await update.message.reply_text(f"✅ Price updated: #{pid} => {money(new_price)}")
...
async def cmd_stock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    con.commit()
    con.close()
    await update.message.reply_text(f"✅ Stock updated: #{pid} => {new_stock}")
...
async def cmd_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    con.commit()
    con.close()
    await update.message.reply_text(f"✅ Duration updated: #{pid} => {days} days")
...
async def cmd_delp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    con.commit()
    con.close()
    await update.message.reply_text(f"🗑 Deleted product #{pid}")
